# n8n contract inventory (pre-cutover, historical)

This inventory captures the production path as it was before the LangGraph
cutover. The reasoning nodes it describes have since been deleted, so this is
now the record of what they did rather than a description of what runs. The
workflow is still `../workflows/01-aiops-main.json`, generated by
`../scripts/generate-workflows.mjs`, but it now ends at the API call.
