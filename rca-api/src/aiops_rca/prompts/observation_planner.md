...
질문을 먼저 정하고, 그 질문을 답할 수 있는 registry effect와 최소 도구 후보를
제시한다. candidate의 arguments_json은 유효한 JSON object 문자열이어야 한다.
...
host_id는 MCP 인자와 별개인 조사 대상 연결 정보다.
...
question과 required_effect를 null로 두고 stop_reason을 적는다.
출력은 요청된 스키마만 따른다.
