...
그래서 **DB에 있는 템플릿은 항상 `templates/`에서 볼 수 있습니다.** 운영 중인
문구를 확인하려면 파일을 열면 되고, git 이력이 곧 변경 이력입니다.
...
Agent의 프롬프트가 되므로, 런타임에 발견되면 이미 접수·응답까지 끝난 질문이
실패합니다. 배포가 지켜보고 있을 때 막는 편이 낫습니다. 이때 이전 컨테이너가
그대로 남으므로 서비스는 계속됩니다.
...
형식은 다음과 같습니다. 오케스트레이터 호스트에서 임시로 넣어볼 때는:
